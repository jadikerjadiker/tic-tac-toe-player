...
When a player hits an opponents hand, you add the two numbers together
...and then that becomes the opponent's hand.
...If it's over 5, the opponent's hand is out.
...
'''
class ChopsticksOverAndOutGame(PPTwoPlayerGame):
    ...
    def makeMove(self, move, playerNum):
        ...
        if move==5:
            ...
            handSum = (otherPlayer[otherPlayerHandIndex] + player[playerHandIndex])%5
            if handSum>5:
                handSum = 0 #if the sum is bigger than 5, the hand is out
            otherPlayer[otherPlayerHandIndex] = handSum
...
if __name__  == "__main__":
    ...
    gamePlayer = PPTwoPlayerGamePlayer(ChopsticksOverAndOutGame)
    while True:
